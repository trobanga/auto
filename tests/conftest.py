...
def isolated_config_manager(temp_home, monkeypatch):
    ...
    # Override the _find_project_config method to prevent finding real project configs
    # but allow it to work normally within test directories
    original_find_project_config = manager._find_project_config

    def mock_find_project_config():
        # Only allow project config discovery if we're in a pytest environment
        temp_path_str = str(manager._user_config_path)
        is_test_env = "/pytest" in temp_path_str or temp_path_str.startswith("/tmp")

        if is_test_env:
            original_find_project_config()
        else:
            manager._project_config_path = None

    manager._find_project_config = mock_find_project_config
