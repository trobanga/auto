...
from auto.workflows.merge_conflicts import MergeConflictError
...
class TestMergeIntegrationWorkflow:
    ...
    async def test_complete_successful_workflow(
        ...
        with patch("auto.workflows.merge.validate_merge_eligibility") as mock_validate, \
            ...
            mock_conflicts.assert_called_once_with(
                456, "testorg", "integration-test-repo"
            )
    ...
    async def test_retry_mechanism_integration(
        ...
        timeout_result = ShellResult(
            returncode=1,
            stdout="",
            stderr="Network timeout occurred",
            command="gh pr merge"
        )
    ...
    @pytest.mark.asyncio
    async def test_all_retries_exhausted_workflow(
        self, integration_repository, integration_config
    ):
        ...
        with patch("auto.workflows.merge.validate_merge_eligibility") as mock_validate, \
            ...
            assert mock_sleep.call_count == 2  # One for each retry

    @pytest.mark.asyncio
    async def test_timeout_integration_workflow(
        self, integration_repository, integration_config
    ):
    ...
    async def test_configuration_integration(
        ...
        with patch("auto.workflows.merge.validate_merge_eligibility") as mock_validate, \
            ...
            # Check that --delete-branch was NOT added to command
...
class TestErrorRecoveryIntegration:
    ...
    @pytest.mark.asyncio
    async def test_github_api_integration_errors(
        self, integration_repository, integration_config
    ):
...
"""
            success_result = ShellResult(0, success_output, "", "gh pr merge")
            mock_run_cmd.return_value = success_result
