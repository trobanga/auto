...
class TestImplementIssueWorkflow:
    ...
    async def test_successful_implementation(self, sample_issue, workflow_state, ai_response):
        ...
        with (
            patch("auto.workflows.implement.Config") as mock_config_class,
            ...
            patch(
                "auto.workflows.implement.apply_ai_changes", new_callable=AsyncMock
            ) as mock_apply,
        ):
            # Mock configuration
            ...
            mock_apply.assert_called_once_with(
                ai_response=ai_response, worktree_path=workflow_state.worktree, config=mock_config
            )
    ...
    async def test_implementation_with_custom_prompt(self, sample_issue, workflow_state):
        ...
        with (
            ...
            mock_ai_integration = MagicMock()
            mock_ai_integration.execute_implementation = AsyncMock(
    ...
    async def test_show_prompt_mode(self, sample_issue, workflow_state):
        ...
        with (
            ...
            result = await implement_issue_workflow(sample_issue, workflow_state, show_prompt=True)
    ...
    async def test_nonexistent_worktree_error(self, sample_issue):
        ...
        workflow_state = WorkflowState(
            issue_id="#123", worktree="/nonexistent/path", status=WorkflowStatus.FETCHING
        )
...
class TestApplyAIChanges:
    ...
    async def test_apply_file_changes(self, ai_response, tmp_path):
        ...
        with (
            patch("auto.workflows.implement.os.chdir") as mock_chdir,
    ...
    async def test_execute_commands(self, tmp_path):
        ...
        with (
            patch("auto.workflows.implement.os.chdir"),
            patch("auto.workflows.implement.os.getcwd", return_value="/original"),
            ...
            mock_subprocess.assert_any_call(
                "echo 'test'", cwd=worktree_path, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
    ...
    async def test_directory_restoration_on_exception(self, ai_response, tmp_path):
        ...
        with (
            ...
            patch("auto.workflows.implement.os.getcwd", return_value="/original"),
            patch(
                "auto.workflows.implement._apply_file_changes", side_effect=Exception("Test error")
            ),
        ):
...
class TestValidateImplementationPrerequisites:
    ...
    def test_no_worktree(self):
        ...
        workflow_state = WorkflowState(
            issue_id="#123", worktree=None, status=WorkflowStatus.FETCHING
        )
    ...
    def test_worktree_not_directory(self, tmp_path):
        ...
        workflow_state = WorkflowState(
            issue_id="#123", worktree=str(worktree_file), status=WorkflowStatus.FETCHING
        )
    ...
    def test_worktree_not_git_repo(self, tmp_path):
        ...
        workflow_state = WorkflowState(
            issue_id="#123", worktree=str(worktree_path), status=WorkflowStatus.FETCHING
        )
...
class TestGetImplementationSummary:
    ...
    def test_not_started_summary(self):
        ...
        workflow_state = WorkflowState(
            issue_id="#123", status=WorkflowStatus.FETCHING, ai_status=AIStatus.NOT_STARTED
        )
    ...
    def test_in_progress_summary(self):
        ...
        workflow_state = WorkflowState(
            issue_id="#123", status=WorkflowStatus.IMPLEMENTING, ai_status=AIStatus.IN_PROGRESS
        )
