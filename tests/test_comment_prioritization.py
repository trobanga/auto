...
class TestCommentCategorizationEdgeCases:
    ...
    def test_mixed_category_comments(self, comment_processor):
        ...
        mixed_comments = [
            (
                "This bug causes a security vulnerability",
                CommentCategory.SECURITY,
            ),  # Security takes priority
            (
                "Performance issue that breaks functionality",
                CommentCategory.BUG,
            ),  # Bug takes priority
            (
                "Style issue that affects performance",
        ...
        for comment_text, expected_category in mixed_comments:
            ...
            assert actual_category == expected_category, (
                f"Comment '{comment_text}' should be {expected_category}, got {actual_category}"
            )
    ...
    def test_critical_priority_keywords(self, comment_processor):
        ...
        for comment_text in critical_keywords:
            ...
            assert priority == CommentPriority.CRITICAL, (
                f"'{comment_text}' should be CRITICAL priority"
            )
    ...
    def test_priority_category_interaction(self, comment_processor):
        ...
        for comment_text, category, expected_priority in test_cases:
            ...
            assert actual_priority == expected_priority, (
                f"'{comment_text}' with category {category} should be {expected_priority}, got {actual_priority}"
            )
...
class TestCommentComplexityScoring:
    ...
    def test_complexity_score_ranges(self, comment_processor):
        ...
        for comment_text, category, (min_score, max_score) in test_cases:
            ...
            assert min_score <= score <= max_score, (
                f"'{comment_text}' complexity score {score} not in range {min_score}-{max_score}"
            )
    ...
    def test_effort_estimation_consistency(self, comment_processor):
        ...
        for complexity_score, expected_effort in test_cases:
            effort = comment_processor._estimate_effort(
                complexity_score, CommentCategory.CODE_QUALITY
            )
            assert effort == expected_effort, (
                f"Complexity {complexity_score} should be '{expected_effort}', got '{effort}'"
            )
...
class TestActionabilityDetection:
    ...
    def test_non_actionable_comments(self, comment_processor):
        ...
        for comment_text, category in non_actionable_comments:
            ...
            assert isinstance(is_actionable, bool), (
                f"Actionability check for '{comment_text}' should return boolean"
            )
    ...
    def test_code_change_requirement_detection(self, comment_processor):
        ...
        for comment_text, comment_type, expected in all_cases:
            ...
            assert requires_change == expected, (
                f"'{comment_text}' code change requirement should be {expected}, got {requires_change}"
            )
