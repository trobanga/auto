...
from auto.models import AIConfig, Config
...
class TestReviewCycleCompletion:
    ...
    def mock_config(self):
        ...
        config.workflows.review_check_interval = 0.1  # Fast polling for tests
    ...
    async def test_cycle_completion_with_no_human_review(self, mock_config):
        ...
        with patch('auto.workflows.review.get_config', return_value=mock_config), \
            ...
            # Execute review cycle with very short timeout
            result = await execute_review_cycle(
                pr_number=123,
                repository="owner/repo"
            )
...
class TestReviewCyclePerformance:
    ...
    def mock_config(self):
        ...
        config.workflows.ai_review_first = True
        config.workflows.review_check_interval = 0.01  # Very fast polling
        config.workflows.require_human_approval = True
        config.ai = AIConfig(
            command="claude",
            implementation_agent="coder",
    ...
    async def test_large_scale_review_cycle_performance(self, mock_config):
        ...
        with patch('auto.workflows.review.get_config', return_value=mock_config), \
            ...
            mock_review.check_approval_status.return_value = (True, [f"reviewer{i}" for i in range(10)], [])
            mock_review_class.return_value = mock_review
    ...
    def test_review_cycle_status_transitions(self):
        ...
        for status in valid_transitions:
            ...
            assert status.value in [
                "pending", "ai_review_in_progress", "waiting_for_human",
                "human_review_received", "ai_update_in_progress", 
                "changes_requested", "approved", "max_iterations_reached", "failed"
            ]
