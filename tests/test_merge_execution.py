...
from auto.workflows.merge_validation import MergeValidationError
...
class TestExecuteMergeOperation:
    ...
    async def test_successful_merge(
        ...
        with patch(
            "auto.workflows.merge.validate_merge_eligibility"
        ) as mock_validate, patch(
            "auto.workflows.merge.handle_merge_conflicts"
        ) as mock_conflicts, patch(
            "auto.workflows.merge.run_command_async"
        ) as mock_run, patch(
            "auto.workflows.merge._extract_merge_commit_sha"
        ) as mock_extract:
            ...
            result = await _execute_merge_operation(
                123, sample_repository, "merge", sample_config
            )
    ...
    async def test_validation_failure(self, sample_repository, sample_config):
        ...
        with patch(
            ...
            # Execute merge
            result = await _execute_merge_operation(
                123, sample_repository, "squash", sample_config
            )
    ...
    async def test_merge_conflicts(self, sample_repository, sample_config):
        ...
        with patch(
            ...
            # Execute merge
            result = await _execute_merge_operation(
                123, sample_repository, "rebase", sample_config
            )
...
class TestExtractMergeCommitSha:
    ...
    async def test_extract_from_gh_output(self):
        ...
        # Test with full SHA
        output_with_full_sha = "✓ Pull request #123 merged (commit abc123def456789012345678901234567890abcd)"
