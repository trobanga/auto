...
class TestMergeAutomation:
    ...
    async def test_execute_auto_merge_success(self, mock_config, mock_pr_info):
        ...
        with patch('auto.workflows.merge.get_config', return_value=mock_config), \
    ...
    async def test_cleanup_after_merge_success(self):
        ...
        with patch('auto.workflows.merge.GitIntegration') as mock_git_class, \
            ...
            mock_git = MagicMock()
            mock_git.remove_worktree = AsyncMock()
            ...
            # Assert
            mock_git.remove_worktree.assert_called_once_with(worktree_path)
...
class TestMergeHelperFunctions:
    ...
    async def test_execute_merge_success(self):
        ...
        with patch('auto.workflows.merge.run_command', return_value=mock_result) as mock_run, \
    ...
    async def test_get_pr_info_success(self):
        ...
        expected_data = {"state": "open", "mergeable": True}
        mock_result = MagicMock()
        mock_result.returncode = 0
...
class TestMergeIntegration:
    ...
    async def test_merge_workflow_with_validation_errors(self):
        ...
        with patch('auto.workflows.merge.get_config', return_value=mock_config), \
             ...
             patch('auto.workflows.merge._validate_status_checks', return_value=(False, ["Failing checks"])), \
             patch('auto.workflows.merge._validate_branch_protection', return_value=(True, [])):
