...
class TestConfigManager:
    ...
    def test_default_config_creation(self, isolated_config_manager):
        """Test default configuration creation."""
        _config_path = isolated_config_manager.create_default_config(user_level=True)
    ...
    def test_env_var_expansion(self, isolated_config_manager, temp_home):
        ...
        try:
            ...
            config_data = {
                "github": {"token": "${TEST_TOKEN}", "default_org": "${MISSING_VAR:-default-org}"},
                "ai": {"implementation_agent": "$TEST_AGENT"},
            }
            ...
            isolated_config_manager._user_config_path = config_path

            # Load config and verify expansion
            config = isolated_config_manager.load_config()
    ...
    def test_env_override(self, isolated_config_manager, temp_home):
        ...
        try:
            # Create basic config
            config_data = {"ai": {"command": "original-claude"}, "defaults": {"auto_merge": False}}
    ...
    def test_config_merge(self, isolated_config_manager, temp_home):
        """Test configuration merging."""
        # Create user config
        user_config = {"ai": {"command": "user-claude"}, "defaults": {"auto_merge": True}}

        user_path = temp_home / ".auto" / "config.yaml"
        ...
        project_path = temp_home / "project" / ".auto" / "config.yaml"
        ...
        isolated_config_manager._user_config_path = user_path
        isolated_config_manager._project_config_path = project_path
    ...
    def test_get_set_config_value(self, isolated_config_manager):
        ...
        # Test getting value
        command = isolated_config_manager.get_config_value("ai.command")
        assert command == "claude"

        # Test setting value
        isolated_config_manager.set_config_value("ai.command", "new-claude", user_level=True)

        # Verify value was set
        new_command = isolated_config_manager.get_config_value("ai.command")
    ...
    def test_invalid_key(self, isolated_config_manager):
        """Test getting invalid configuration key."""
        isolated_config_manager.create_default_config(user_level=True)

        with pytest.raises(ConfigError):
            isolated_config_manager.get_config_value("invalid.key.path")

    def test_config_file_listing(self, isolated_config_manager):
        """Test configuration file listing."""
        # Initially no files
        files = isolated_config_manager.list_config_files()
