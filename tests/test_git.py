...
def sample_worktree_info():
    ...
    return WorktreeInfo(path="/tmp/test-worktree", branch="auto/feature/123", issue_id="#123")
...
class TestWorktreePath:
    ...
    @patch("auto.integrations.git.get_main_repo_root")
    def test_generate_worktree_path(self, mock_get_main_repo_root, mock_config):
        """Test worktree path generation."""
        mock_get_main_repo_root.return_value = Path("/repo")
...
class TestWorktreeCreation:
    ...
    def test_create_worktree_success(
        self, mock_run_command, mock_get_git_root, mock_config, sample_issue
    ):
    ...
    @patch("auto.integrations.git.get_git_root")
    @patch("auto.integrations.git.run_command")
    def test_create_worktree_branch_conflict(
        self, mock_run_command, mock_get_git_root, mock_config, sample_issue
    ):
        ...
        mock_run_command.side_effect = [
            ShellResult(
                0, "auto/feature/123", "", "git branch --list auto/feature/123"
        ...
        with patch.object(Path, "exists", return_value=False):
            ...
            with pytest.raises(
                GitWorktreeConflictError, match="Branch auto/feature/123 already exists"
            ):
                manager.create_worktree(sample_issue)

    @patch("auto.integrations.git.get_git_root")
    @patch("auto.integrations.git.run_command")
    def test_create_worktree_force_branch_conflict(
        self, mock_run_command, mock_get_git_root, mock_config, sample_issue
    ):
    ...
    @patch("auto.integrations.git.get_git_root")
    @patch("auto.integrations.git.run_command")
    def test_create_worktree_path_conflict(
        self, mock_run_command, mock_get_git_root, mock_config, sample_issue
    ):
    ...
    @patch("auto.integrations.git.get_git_root")
    @patch("auto.integrations.git.run_command")
    def test_create_worktree_base_branch_not_found(
        self, mock_run_command, mock_get_git_root, mock_config, sample_issue
    ):
        ...
        mock_run_command.side_effect = [
            ShellResult(
                0, "", "", "git branch --list auto/feature/123"
            ),  # branch conflict check (no conflict)
            ShellResult(0, "", "", "git fetch"),  # fetch
            ShellResult(0, "", "", "git branch --list main"),  # base branch doesn't exist locally
            ShellResult(
                0, "", "", "git ls-remote --heads origin main"
    ...
    def test_create_worktree_git_failure(
        ...
        mock_run_command.side_effect = [
            ...
            ShellResult(0, "", "", "git branch --list auto/feature/123"),  # branch conflict check
            ShellError(
                "Git worktree add failed", 1, "", "fatal: could not create worktree"
...
class TestWorktreeCleanup:
    ...
    def test_cleanup_worktree_success(
        self, mock_run_command, mock_get_git_root, mock_config, sample_worktree_info
    ):
    ...
    def test_cleanup_worktree_force_removal(
        self, mock_shutil, mock_run_command, mock_get_git_root, mock_config, sample_worktree_info
    ):
    ...
    def test_cleanup_worktree_manual_removal(
        self, mock_rmtree, mock_run_command, mock_get_git_root, mock_config, sample_worktree_info
    ):
    ...
    @patch("auto.integrations.git.get_git_root")
    @patch("auto.integrations.git.run_command")
    def test_cleanup_worktree_errors(
        self, mock_run_command, mock_get_git_root, mock_config, sample_worktree_info
    ):
...
"""
        ...
        mock_run_command.return_value = ShellResult(
            0, worktree_output.strip(), "", "git worktree list --porcelain"
        )
...
"""
    ...
    def test_list_worktrees_git_failure(self, mock_run_command, mock_get_git_root, mock_config):
        ...
        mock_run_command.side_effect = ShellError(
            "Git failed", 1, "", "fatal: not a git repository"
        )
