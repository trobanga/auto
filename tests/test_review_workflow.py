...
class TestReviewCycleOrchestration:
    ...
    async def test_execute_review_cycle_max_iterations(self, mock_config):
        ...
        with (
            ...
            patch(
                ...
                return_value=ReviewCycleStatus.CHANGES_REQUESTED,
            ),
        ):
            # Execute review cycle
            result = await execute_review_cycle(pr_number=123, repository="owner/repo")
    ...
    async def test_execute_review_cycle_error_handling(self, mock_config):
        ...
        with (
            patch("auto.workflows.review.get_config", return_value=mock_config),
            patch(
                "auto.workflows.review.trigger_ai_review", side_effect=Exception("AI review failed")
            ),
        ):
...
class TestHumanReviewMonitoring:
    ...
    async def test_wait_for_human_review_success(self, sample_pr_reviews):
        ...
        with (
            patch("auto.workflows.review.get_config") as mock_get_config,
            patch("auto.workflows.review.GitHubReviewIntegration") as mock_review_class,
        ):
            # Setup config
            mock_config = Mock()
            mock_config.workflows.review_check_interval = 1
...
class TestReviewCommentProcessing:
    ...
    async def test_process_review_comments(self, sample_review_comments):
        ...
        with patch("auto.workflows.review.GitHubReviewIntegration") as mock_review_class:
            ...
            assert (
                state.unresolved_comments[0].body
