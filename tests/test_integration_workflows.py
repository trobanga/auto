...
from auto.models import Config, WorkflowConfig, AIConfig, GitHubConfig
...
class TestCompleteReviewWorkflows:
    ...
    async def test_simple_approval_workflow(self, full_config):
        ...
        with patch('auto.workflows.review.get_config', return_value=full_config), \
            ...
            mock_ai = Mock()
            mock_ai.execute_review = AsyncMock(return_value=AIResponse(
                success=True,
    ...
    async def test_iterative_improvement_workflow(self, full_config):
        ...
        with patch('auto.workflows.review.get_config', return_value=full_config), \
            ...
            mock_ai.execute_update_from_review = AsyncMock(return_value=AIResponse(
                success=True,
    ...
    async def test_complex_multi_reviewer_workflow(self, full_config):
        ...
        with patch('auto.workflows.review.get_config', return_value=full_config), \
            ...
            # Setup GitHub integration with multiple reviewers
            mock_github = Mock()
            mock_github.post_ai_review.return_value = Mock(id=1, state="COMMENTED")
            ...
            def get_reviews_side_effect(*args, **kwargs):
                ...
                if iteration_count[0] == 1:  # First check - mixed reviews
                    return [
                        PRReview(
                            id=2,
                            author="reviewer_1",
                ...
                elif iteration_count[0] == 2:  # Second check - more reviews
                    return [
                        PRReview(
                            id=2,
                            author="reviewer_1",
                        ...
                        PRReview(
                            ...
                            state="CHANGES_REQUESTED",
                            body="Please address the performance concerns",
                            submitted_at=datetime.now() - timedelta(minutes=13)
                        ),
                        PRReview(
                            id=4,
                            author="reviewer_3",
                            state="COMMENTED",
                            body="Some minor suggestions",
                            submitted_at=datetime.now() - timedelta(minutes=11)
                        ),
                        PRReview(
                            id=5,
                            author="reviewer_2",  # Same reviewer, updated opinion
                            state="APPROVED",
            ...
            def check_approval_side_effect(*args, **kwargs):
                if iteration_count[0] <= 2:
                    return (False, ["reviewer_1"], ["reviewer_2"])
                else:
                    return (True, ["reviewer_1", "reviewer_2", "reviewer_3"], [])
            ...
            assert len(result.human_reviews) >= 3  # Multiple reviewers captured
            
            # Verify all reviewers eventually approved
            final_reviewers = [review["author"] for review in result.human_reviews 
                             if review.get("state") == "APPROVED"]
            assert "reviewer_1" in final_reviewers
            assert "reviewer_2" in final_reviewers  # Changed opinion
            assert "reviewer_3" in final_reviewers
...
class TestWorkflowPerformance:
    ...
    def performance_config(self):
        ...
        return Config(
            workflows=WorkflowConfig(
                ...
                review_check_interval=0.1,  # Very fast polling
                ai_review_first=True,
                require_human_approval=True
            ),
            ai=AIConfig(
    ...
    async def test_large_pr_review_performance(self, performance_config):
        ...
        with patch('auto.workflows.review.get_config', return_value=performance_config), \
             patch('auto.workflows.review.ClaudeIntegration') as mock_ai_class, \
             patch('auto.workflows.review.GitHubReviewIntegration') as mock_github_class:
            ...
            mock_github.get_unresolved_comments.return_value = large_comments[:10]  # 10 unresolved
...
class TestWorkflowEdgeCases:
    ...
    def edge_case_config(self):
        ...
        return Config(
            workflows=WorkflowConfig(
                ...
                review_check_interval=0.05,  # Very fast
                ai_review_first=True,
                require_human_approval=True
            ),
            ai=AIConfig(
    ...
    async def test_empty_pr_workflow(self, edge_case_config):
        ...
        with patch('auto.workflows.review.get_config', return_value=edge_case_config), \
             patch('auto.workflows.review.ClaudeIntegration') as mock_ai_class, \
             patch('auto.workflows.review.GitHubReviewIntegration') as mock_github_class:
    ...
    async def test_very_long_review_cycle(self, edge_case_config):
        ...
        with patch('auto.workflows.review.get_config', return_value=edge_case_config), \
            ...
            # Always request changes (persistent issue)
            mock_github.get_pr_reviews.return_value = [
                PRReview(
                    id=2,
    ...
    async def test_reviewer_changes_mind_workflow(self, edge_case_config):
        ...
        with patch('auto.workflows.review.get_config', return_value=edge_case_config), \
            ...
            def get_reviews_side_effect(*args, **kwargs):
                ...
                # Reviewer changes mind multiple times
                reviews = [
                    PRReview(
                        id=2,
                        author="indecisive_reviewer",
                        state="APPROVED",
                        body="Initially looks good",
                        submitted_at=datetime.now() - timedelta(minutes=10)
                    )
                ]
                ...
                if call_sequence[0] > 2:
                    reviews.append(PRReview(
                        ...
                        author="indecisive_reviewer",
                        state="APPROVED",
                        body="Actually, it's fine. My mistake.",
                        submitted_at=datetime.now()
                    ))
            ...
            def check_approval_side_effect(*args, **kwargs):
                if call_sequence[0] == 2:  # Middle state - changes requested
                    return (False, [], ["indecisive_reviewer"])
                else:  # Initial and final state - approved
