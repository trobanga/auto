...
@pytest.fixture
def update_workflow(
    mock_github_integration, mock_git_integration, mock_ai_integration, mock_comment_processor
):
    ...
    return [
        ProcessedComment(
            original_comment=ReviewComment(
                id=1, body="Bug in authentication", path="src/auth.py", line=15, author="reviewer"
            ),
        ...
        ProcessedComment(
            original_comment=ReviewComment(
                id=2, body="Performance issue", path="src/auth.py", line=25, author="reviewer"
            ),
        ...
        ProcessedComment(
            original_comment=ReviewComment(
                id=3, body="Missing documentation", path="src/utils.py", line=10, author="reviewer"
            ),
...
class TestUpdatePlanCreation:
    ...
    @pytest.mark.asyncio
    async def test_create_update_plans(
        self, update_workflow, sample_processing_result, sample_issue
    ):
    ...
    async def test_create_file_update_plans(self, update_workflow, sample_issue):
        ...
        style_comment = ProcessedComment(
            original_comment=ReviewComment(
                id=2, body="Style issue", path="src/auth.py", author="reviewer"
            ),
...
class TestUpdateBatchOrganization:
    ...
    async def test_organize_update_batches(self, update_workflow):
        ...
        if dependent_batches:
            ...
            plan3_found = any(
                plan3.update_id in batch.execution_order for batch in dependent_batches
            )
...
class TestUpdateExecution:
    ...
    @pytest.mark.asyncio
    async def test_execute_single_update_success(
        self, update_workflow, mock_ai_integration, sample_issue
    ):
        ...
        with patch.object(
            update_workflow, "_run_update_validations", return_value={"syntax_check": True}
        ):
    ...
    @pytest.mark.asyncio
    async def test_execute_single_update_failure(
        self, update_workflow, mock_ai_integration, sample_issue
    ):
...
class TestUpdateValidation:
    ...
    async def test_run_update_validations(self, update_workflow):
        ...
        with (
            patch.object(update_workflow, "_validate_syntax", return_value=True),
...
class TestCommitStrategy:
    ...
    async def test_commit_review_changes(self, update_workflow, mock_git_integration):
        ...
        with patch("auto.utils.shell.run_command") as mock_run_command:
            # Mock to return an object with stdout attribute
            mock_result = Mock()
            ...
            # Verify shell commands were called (actual implementation uses shell commands)
            assert mock_run_command.call_count >= 3  # git add calls + git commit + git rev-parse
    ...
    async def test_group_updates_for_commits(self, update_workflow):
        ...
        single_groups = await update_workflow._group_updates_for_commits(
            update_results, single_strategy
        )
    ...
    async def test_generate_commit_message(self, update_workflow):
        ...
        message = await update_workflow._generate_commit_message(update_group, commit_strategy, 123)
...
class TestPRUpdateIntegration:
    ...
    @pytest.mark.asyncio
    async def test_update_pr_with_changes(
        self, update_workflow, mock_git_integration, mock_github_integration
    ):
        ...
        with patch("auto.utils.shell.run_command") as mock_run_command:
            ...
            # Verify shell commands were called (actual implementation uses shell)
            assert mock_run_command.call_count >= 2  # git branch --show-current + git push
    ...
    async def test_add_update_comment_to_pr(self, update_workflow, mock_github_integration):
        ...
        await update_workflow._add_update_comment_to_pr(123, "test/repo", update_results)
        ...
        assert (
            "Bug" in comment_body
        )  # The implementation uses the first part of update_id, which is "bug"
...
class TestEndToEndWorkflow:
    ...
    async def test_execute_review_updates_integration(
        ...
        # Mock validation to always pass and shell commands
        with (
            patch.object(update_workflow, "_validate_all_updates") as mock_validate,
            patch("auto.utils.shell.run_command") as mock_run_command,
        ):
        ...
        # Check that commits were created via shell commands (since validation passed)
        assert mock_run_command.call_count >= 3  # git add, git commit, git rev-parse calls
