...
class TestCLIReviewCommands:
    ...
    def test_review_command_success(self):
        ...
        with (
            patch("auto.integrations.github.detect_repository", return_value=self.mock_repo),
            patch("auto.workflows.review.trigger_ai_review") as mock_trigger,
        ):
            # Create a proper async function to avoid AsyncMock issues
            async def mock_async_function(*args, **kwargs):
                return None

            mock_trigger.side_effect = mock_async_function
    ...
    def test_review_command_with_options(self):
        ...
        with (
            ...
            result = self.runner.invoke(
                cli, ["review", "123", "--force", "--agent", "custom-reviewer"]
            )
            ...
            # (In a real test we could verify the state object passed to the function)

    def test_review_command_invalid_pr_id(self):
        """Test review command with invalid PR ID."""
        with patch("auto.integrations.github.detect_repository", return_value=self.mock_repo):
    ...
    def test_review_command_repository_detection_failure(self):
        ...
        with patch(
            "auto.integrations.github.detect_repository", side_effect=Exception("No git repo")
        ):
            # Execute command
            result = self.runner.invoke(cli, ["review", "123"])
    ...
    def test_review_command_trigger_failure(self):
        ...
        with (
            patch("auto.integrations.github.detect_repository", return_value=self.mock_repo),
            patch("asyncio.run", side_effect=Exception("AI review failed")),
        ):
            # Execute command
            result = self.runner.invoke(cli, ["review", "123"])
    ...
    def test_update_command_success(self):
        ...
        with (
            patch("auto.integrations.github.detect_repository", return_value=self.mock_repo),
            patch("auto.workflows.review_update.execute_review_update") as mock_update,
            ...
            # Assert
            assert result.exit_code == 0
            assert "Updating PR #123" in result.output
    ...
    def test_update_command_with_options(self):
        ...
        with (
            patch("auto.integrations.github.detect_repository", return_value=self.mock_repo),
            patch("auto.workflows.review_update.execute_review_update", return_value=None),
            patch("asyncio.run", return_value=True) as mock_asyncio,
        ):
            # Execute command with options
            result = self.runner.invoke(
                cli, ["update", "123", "--force", "--agent", "custom-updater"]
            )
    ...
    def test_merge_command_success(self):
        ...
        with (
            patch("auto.integrations.github.detect_repository", return_value=self.mock_repo),
            patch("auto.workflows.merge.execute_auto_merge") as mock_merge,
    ...
    def test_merge_command_with_options(self):
        ...
        with (
            patch("auto.integrations.github.detect_repository", return_value=self.mock_repo),
            patch("auto.workflows.merge.execute_auto_merge", return_value=None),
            patch("auto.cli.get_core"),
            patch("asyncio.run", return_value=True) as mock_asyncio,
        ):
            # Execute command with options
            result = self.runner.invoke(
                cli, ["merge", "123", "--force", "--method", "squash", "--no-cleanup"]
            )
    ...
    def test_merge_command_failure(self):
        ...
        with (
            patch("auto.integrations.github.detect_repository", return_value=self.mock_repo),
            patch("auto.cli.get_core") as _mock_core,
            patch("asyncio.run", return_value=False),
        ):
            # Execute command
            result = self.runner.invoke(cli, ["merge", "123"])
...
class TestCLIStatusCommand:
    ...
    def setup_method(self):
        ...
        # Use simple Mock without spec to avoid auto-generating problematic methods
        self.mock_workflow_state = MagicMock()
        self.mock_workflow_state.issue_id = "TEST-123"

        # Create proper status mocks with .value attributes
        status_mock = Mock()
        status_mock.value = "in_review"
        self.mock_workflow_state.status = status_mock

        ai_status_mock = Mock()
        ai_status_mock.value = "implemented"
        self.mock_workflow_state.ai_status = ai_status_mock
        ...
        mock_updated_at.strftime.return_value = "2024-01-15 10:30"
        self.mock_workflow_state.updated_at = mock_updated_at
    ...
    def test_status_command_with_reviews(self):
        ...
        with (
            patch("auto.cli.get_core", return_value=self.mock_core),
            patch("auto.workflows.review.get_review_cycle_status", return_value=None),
            patch("asyncio.run") as mock_asyncio,
        ):
            self.mock_core.get_workflow_states.return_value = [self.mock_workflow_state]
            ...
            # Execute command (no --reviews flag, just regular status)
            result = self.runner.invoke(cli, ["status"])
    ...
    def test_status_command_verbose(self):
        ...
        with patch("auto.cli.get_core", return_value=self.mock_core):
            ...
            # Assert
            assert result.exit_code == 0
            assert "Worktr" in result.output  # Truncated in table
            assert "Reposit" in result.output  # Truncated in table
...
class TestCLIIntegration:
    ...
    def test_review_update_merge_workflow(self):
        ...
        with (
            patch("auto.integrations.github.detect_repository", return_value=mock_repo),
            patch("asyncio.run", return_value=True),
        ):
    ...
    def test_error_propagation(self):
        ...
        with patch(
            "auto.integrations.github.detect_repository", side_effect=Exception("Repository error")
        ):
...
class TestCLIArgumentValidation:
    ...
    def test_boolean_flag_handling(self):
        ...
        with (
            patch("auto.integrations.github.detect_repository") as mock_detect,
            patch("auto.workflows.review.trigger_ai_review", return_value=None),
            patch("auto.workflows.merge.execute_auto_merge", return_value=None),
            patch("auto.cli.get_core"),
            patch("asyncio.run", return_value=True),
        ):
