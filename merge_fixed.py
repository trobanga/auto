...
async def _execute_merge_operation(
    ...
    try:
        ...
        if conflict_files:
            from auto.workflows.merge_conflicts import get_conflict_details
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if shell_result.success:
                    ...
                    merge_sha = await _extract_merge_commit_sha(shell_result.stdout, repository, pr_number)
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"Merge attempt {attempt + 1} timed out, retrying in {retry_delay}s")
                    await asyncio.sleep(retry_delay)
                    continue
                else:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"Merge attempt {attempt + 1} failed with exception, retrying: {e}")
                    await asyncio.sleep(retry_delay)
                    continue
                else:
