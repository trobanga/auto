...
from ..integrations.ai import AIIntegrationError, ClaudeIntegration
from ..integrations.prompts import PromptError, PromptManager
from ..models import AIResponse, AIStatus, Issue, WorkflowState, WorkflowStatus
...
async def implement_issue_workflow(
    ...
    try:
        ...
        if not workflow_state.worktree:
            raise ImplementationError("No worktree available for implementation") from None

        if not Path(workflow_state.worktree).exists():
            raise ImplementationError(
                f"Worktree path does not exist: {workflow_state.worktree}"
            ) from None
        ...
        try:
            ...
            raise ImplementationError(f"Failed to resolve prompt: {e}") from e
        ...
        ai_response = await ai_integration.execute_implementation(
            issue=issue, worktree_path=workflow_state.worktree, custom_prompt=resolved_prompt
        )
        ...
        if ai_response.success:
            ...
            await apply_ai_changes(
                ai_response=ai_response, worktree_path=workflow_state.worktree, config=config
            )
            ...
            raise ImplementationError(f"AI implementation failed: {ai_response.content}") from None
        ...
        workflow_state.update_status(WorkflowStatus.FAILED)
        raise ImplementationError(f"AI integration failed: {e}") from e

    except Exception as e:
        logger.error(f"Unexpected error during implementation: {e}")
        workflow_state.update_ai_status(AIStatus.FAILED)
        workflow_state.update_status(WorkflowStatus.FAILED)
        raise ImplementationError(f"Implementation failed: {e}") from e


async def apply_ai_changes(ai_response: AIResponse, worktree_path: str, config: Config) -> None:
    """
    ...
    try:
        ...
        raise ImplementationError(f"Failed to apply changes: {e}") from e
...
async def _apply_file_changes(file_changes: list[dict[str, str]], worktree_path: str) -> None:
...
def validate_implementation_prerequisites(workflow_state: WorkflowState) -> None:
    ...
    """
    if not workflow_state.worktree:
        raise ImplementationError("No worktree configured for implementation") from None
    ...
    if not worktree_path.is_dir():
        raise ImplementationError(
            f"Worktree path is not a directory: {workflow_state.worktree}"
        ) from None
    ...
    if not config.ai.implementation_agent:
        raise ImplementationError("No AI implementation agent configured") from None
