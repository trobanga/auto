...
class ProcessedComment(BaseModel):
    ...
    suggested_change: Optional[str] = Field(default=None, description="Suggested code change if applicable")
    ...
    related_files: List[str] = Field(default_factory=list, description="Files that might be affected")
    dependencies: List[int] = Field(default_factory=list, description="IDs of comments this depends on")
...
class CommentResponse(BaseModel):
    ...
    planned_action: Optional[str] = Field(default=None, description="Planned action to address comment")
    implementation_notes: Optional[str] = Field(default=None, description="Implementation details")
    requires_discussion: bool = Field(default=False, description="Whether comment needs human discussion")
...
class CommentThread(BaseModel):
    ...
    related_comments: List[ProcessedComment] = Field(default_factory=list, description="Related comments")
    responses: List[CommentResponse] = Field(default_factory=list, description="AI responses in thread")
...
class ReviewCommentProcessor:
    ...
    async def generate_comment_responses(
        ...
        """
        self.logger.info(f"Generating responses for {processing_result.actionable_count} actionable comments")
        ...
        try:
            ...
            for comment_id in processing_result.recommended_order:
                comment = next((c for c in processing_result.processed_comments if c.original_comment.id == comment_id), None)
    ...
    def prioritize_feedback(self, processed_comments: List[ProcessedComment]) -> List[ProcessedComment]:
        """
    ...
    def _determine_priority(self, comment_text: str, category: CommentCategory) -> CommentPriority:
        ...
        # Critical indicators
        if any(word in comment_lower for word in ['critical', 'urgent', 'blocking', 'broken', 'security']):
    ...
    def _analyze_comment_type(self, comment: ReviewComment) -> CommentType:
        ...
        if any(word in comment.body.lower() for word in ['must', 'required', 'needs', 'should fix']):
    ...
    def _extract_keywords(self, comment_text: str) -> List[str]:
        ...
        # Technical terms
        tech_terms = re.findall(r'\b(?:async|await|promise|callback|cache|database|api|http|json|xml|sql)\b', 
                               comment_text.lower())
        keywords.extend(tech_terms)
        
        # Code elements
        code_elements = re.findall(r'\b(?:function|method|class|variable|parameter|return|exception)\b', 
                                 comment_text.lower())
    ...
    def _identify_related_files(self, comment: ReviewComment, pr_number: int, repository: str) -> List[str]:
    ...
    def _organize_comment_threads(self, processed_comments: List[ProcessedComment]) -> List[CommentThread]:
    ...
    def _group_comments_by_proximity(self, comments: List[ProcessedComment]) -> List[List[ProcessedComment]]:
        """Group comments by line number proximity."""
        # Sort by line number
        sorted_comments = sorted(
            [c for c in comments if c.original_comment.line is not None],
            key=lambda c: c.original_comment.line
        )
        ...
        for comment in sorted_comments[1:]:
            ...
            if comment.original_comment.line - current_group[-1].original_comment.line <= 10:
    ...
    def _calculate_priority_summary(self, processed_comments: List[ProcessedComment]) -> Dict[CommentPriority, int]:
        """Calculate summary of comments by priority."""
        summary = {priority: 0 for priority in CommentPriority}
        for comment in processed_comments:
            summary[comment.priority] += 1
        return summary

    def _calculate_category_summary(self, processed_comments: List[ProcessedComment]) -> Dict[CommentCategory, int]:
        """Calculate summary of comments by category."""
        summary = {category: 0 for category in CommentCategory}
        for comment in processed_comments:
    ...
    def _estimate_total_effort(self, processed_comments: List[ProcessedComment]) -> str:
        ...
        total_score = sum(effort_scores.get(comment.estimated_effort, 3) for comment in processed_comments if comment.actionable)
...
Keep the response professional, concise, and constructive."""
            ...
            if not ai_result.success:
                self.logger.warning(f"Failed to generate response for comment {comment.original_comment.id}")
                return None
            ...
            requires_discussion = any(phrase in response_text.lower() for phrase in [
                'discuss', 'clarification', 'question', 'not sure', 'unclear'
            ])
            ...
            self.logger.error(f"Failed to generate response for comment {comment.original_comment.id}: {e}")
            return None

    def _extract_planned_action(self, response_text: str) -> Optional[str]:
    ...
    def _extract_implementation_notes(self, response_text: str) -> Optional[str]:
    ...
    async def _generate_resolution_summary(self, thread: CommentThread) -> str:
        ...
        if len(all_comments) == 1:
            ...
            categories = list(set(c.category.value for c in all_comments))
    ...
    async def _save_comment_history(self, history: CommentHistory) -> None:
        ...
        try:
            ...
            # Save history file
            history_file = history_dir / f"pr_{history.pr_number}_{history.processing_timestamp.isoformat()}.json"
