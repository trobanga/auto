...
async def fetch_issue_workflow(issue_id: str) -> WorkflowState:
    ...
    try:
        ...
        if identifier.provider == IssueProvider.GITHUB:
            ...
            raise FetchWorkflowError("Linear integration not yet implemented") from None
        else:
            raise FetchWorkflowError(f"Unsupported issue provider: {identifier.provider}") from None
        ...
        raise FetchWorkflowError(f"Failed to fetch issue {issue_id}: {e}") from e
...
async def _fetch_github_issue(issue_id: str) -> Issue:
    ...
    try:
        ...
        raise FetchWorkflowError(f"GitHub integration error: {e}") from e
    except Exception as e:
        raise FetchWorkflowError(f"Unexpected error fetching GitHub issue: {e}") from e
...
def get_issue_from_state(issue_id: str) -> Issue | None:
