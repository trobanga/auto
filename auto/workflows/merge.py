...
from auto.integrations.git import GitIntegration, GitWorktreeError
from auto.integrations.github import GitHubIntegration, GitHubError
...
from auto.utils.shell import CommandError, run_command
...
async def execute_auto_merge(
    ...
    try:
        ...
        if conflicts and not force:
            ...
            raise MergeConflictError(f"Merge conflicts detected: {conflicts}")
        
        # Execute the merge
        merge_method = config.defaults.merge_method if hasattr(config.defaults, 'merge_method') else "merge"
        success = await _execute_merge(pr_number, owner, repo, merge_method)
        
        if not success:
            raise MergeExecutionError(f"Failed to merge PR #{pr_number}")
...
async def _execute_merge(
    ...
    try:
        ...
        result = await run_command(cmd, capture_output=True)
...
async def _get_pr_info(pr_number: int, owner: str, repo: str) -> Dict:
    """Get PR information from GitHub."""
    try:
        cmd = [
            "gh", "pr", "view", str(pr_number),
            "--repo", f"{owner}/{repo}",
            "--json", "state,draft,mergeable,reviews,statusCheckRollup,baseRefName"
        ]
