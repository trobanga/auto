...
from ..models import Issue, PRMetadata, PullRequest, WorkflowState, WorkflowStatus
...
def truncate_pr_description(description: str) -> str:
    ...
    logger.warning(
        f"PR description is {len(description)} characters, truncating to fit GitHub's 65536 limit"
    )
...
async def create_pull_request_workflow(
    issue: Issue, workflow_state: WorkflowState, draft: bool = False, auto_merge: bool = False
) -> WorkflowState:
    """
    ...
    try:
        ...
        if workflow_state.worktree is None:
            raise PRCreationError("No worktree path available") from None
        if workflow_state.branch is None:
            raise PRCreationError("No branch name available") from None

        # mypy type narrowing
        worktree_path = workflow_state.worktree
        branch_name = workflow_state.branch
        ...
        if not has_uncommitted_changes(worktree_path):
            ...
            if not has_implementation_commits(worktree_path, branch_name):
                raise PRCreationError("No implementation changes found to create PR") from None
        ...
        await commit_implementation_changes(
            issue=issue, workflow_state=workflow_state, config=config
        )
        ...
        raise PRCreationError(f"GitHub PR creation failed: {e}") from None

    except Exception as e:
        logger.error(f"Unexpected error during PR creation: {e}")
        workflow_state.update_status(WorkflowStatus.FAILED)
        raise PRCreationError(f"PR creation failed: {e}") from None


async def generate_pr_metadata(
    issue: Issue, workflow_state: WorkflowState, config: Config, draft: bool = False
) -> PRMetadata:
    """
    ...
    return PRMetadata(
        ...
        reviewers=reviewers,
        draft=draft,
        base_branch=config.github.default_branch
        if hasattr(config.github, "default_branch")
        else "main",
    )
...
async def generate_pr_description(
    issue: Issue, workflow_state: WorkflowState, config: Config
) -> str:
    """
    ...
    try:
        ...
        if workflow_state.worktree is None:
            raise ValueError("Worktree path not available for PR description generation")
...
def _generate_fallback_pr_description(
    ...
    if workflow_state.ai_response:
        ...
        if workflow_state.ai_response.file_changes:
            description_parts.append(
                f"- Modified {len(workflow_state.ai_response.file_changes)} files"
            )

        if workflow_state.ai_response.commands:
            description_parts.append(
                f"- Executed {len(workflow_state.ai_response.commands)} commands"
            )
...
def load_pr_template(config: Config) -> str | None:
    """
    ...
    if workflow_state.ai_response:
        ...
        if file_changes:
            ...
            doc_files = [
                ...
                for f in file_changes
                if any(
                    doc_pattern in f.get("path", "") for doc_pattern in ["README", "docs/", ".md"]
                )
            ]
            if doc_files:
...
async def commit_implementation_changes(
    ...
    try:
        ...
        if result.returncode != 0:
            raise PRCreationError(f"Failed to stage changes: {result.stderr}") from None
        ...
        if result.returncode != 0:
            if "nothing to commit" in result.stdout:
                ...
                raise PRCreationError(f"Failed to commit changes: {result.stderr}") from None
        else:
            logger.info(f"Successfully committed changes: {commit_message}")

    except subprocess.TimeoutExpired:
        raise PRCreationError("Git commit command timed out") from None
    except subprocess.SubprocessError as e:
        raise PRCreationError(f"Git commit command failed: {e}") from e


def generate_commit_message(issue: Issue, workflow_state: WorkflowState, config: Config) -> str:
    """
...
async def push_branch_to_remote(workflow_state: WorkflowState) -> None:
    ...
    """
    if workflow_state.branch is None:
        raise PRCreationError("No branch name available for push") from None
    if workflow_state.worktree is None:
        raise PRCreationError("No worktree path available for push") from None
    ...
    try:
        ...
        if result.returncode != 0:
            raise PRCreationError(f"Failed to push branch: {result.stderr}") from None

        logger.info(f"Successfully pushed branch {workflow_state.branch}")

    except subprocess.TimeoutExpired:
        raise PRCreationError("Git push command timed out") from None
    except subprocess.SubprocessError as e:
        raise PRCreationError(f"Git push command failed: {e}") from e


async def create_github_pr(
    github_integration: GitHubIntegration, pr_metadata: PRMetadata, workflow_state: WorkflowState
) -> PullRequest:
    """
    ...
    """
    if workflow_state.branch is None:
        raise PRCreationError("No branch name available for PR creation") from None
    ...
    try:
        ...
        raise PRCreationError(f"Failed to create GitHub PR: {e}") from e
...
def validate_pr_prerequisites(workflow_state: WorkflowState) -> None:
    ...
    if not workflow_state.worktree:
        raise PRCreationError("No worktree available for PR creation") from None

    worktree_path = Path(workflow_state.worktree)
    if not worktree_path.exists():
        raise PRCreationError(f"Worktree does not exist: {workflow_state.worktree}") from None

    if not workflow_state.branch:
        raise PRCreationError("No branch specified for PR creation") from None

    # Check if worktree is a git repository
    git_dir = worktree_path / ".git"
    if not git_dir.exists():
        raise PRCreationError(
            f"Worktree is not a git repository: {workflow_state.worktree}"
        ) from None
