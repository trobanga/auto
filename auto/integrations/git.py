...
from auto.models import Issue, IssueType, WorktreeInfo
from auto.utils.logger import get_logger
from auto.utils.shell import ShellError, get_git_root, get_main_repo_root, run_command
...
class GitWorktreeManager:
    ...
    def _validate_git_repository(self) -> None:
        ...
        if git_root is None:
            raise GitWorktreeError(
                "Not in a git repository. Git worktrees require a git repository."
            )
    ...
    def create_worktree(self, issue: Issue, base_branch: str = "main") -> WorktreeInfo:
        ...
        try:
            ...
            raise GitWorktreeError(f"Failed to create worktree for {issue.id}: {e}") from e
    ...
    def cleanup_worktree(self, worktree_info: WorktreeInfo) -> None:
        ...
        try:
            if worktree_info.exists():
                ...
                if not result.success:
                    ...
                    result = run_command(
                        f"git worktree remove --force {worktree_info.path}", check=False
                    )
                    if not result.success:
                        ...
                        if worktree_info.path_obj.exists():
                            ...
                            logger.warning(
                                f"Manually removed worktree directory: {worktree_info.path}"
                            )
        except Exception as e:
        ...
        try:
            ...
            if result.success and result.stdout.strip():
                ...
                if not result.success:
                    errors.append(
                        f"Failed to delete branch {worktree_info.branch}: {result.stderr}"
                    )
        except Exception as e:
    ...
    def _parse_worktree_entry(self, entry: dict) -> WorktreeInfo | None:
    ...
    def _extract_issue_id_from_branch(self, branch: str) -> str | None:
        ...
        try:
            ...
            if result.success and result.stdout.strip():
                ...
                if strategy == "force":
                    ...
                    raise GitWorktreeConflictError(
                        f"Branch {branch_name} already exists (skipped due to configuration)"
                    )
                else:
                    # Default: raise error (prompt would be handled by CLI)
                    raise GitWorktreeConflictError(f"Branch {branch_name} already exists") from None
        except ShellError as e:
            if "not found" not in str(e).lower():
                raise GitWorktreeError(f"Failed to check existing branch: {e}") from e
    ...
    def _handle_existing_worktree_path(self, worktree_path: Path) -> None:
        ...
        if worktree_path.exists():
            ...
            if strategy == "force":
                ...
                raise GitWorktreeConflictError(
                    f"Worktree path {worktree_path} already exists (skipped due to configuration)"
                )
            else:
                # Default: raise error (prompt would be handled by CLI)
                raise GitWorktreeConflictError(
        ...
        try:
            ...
            if not result.success or not result.stdout.strip():
                ...
                if result.success and result.stdout.strip():
                    ...
                    raise GitWorktreeError(
                        f"Base branch '{base_branch}' not found locally or on remote"
                    )

        except ShellError as e:
            raise GitWorktreeError(f"Failed to prepare base branch '{base_branch}': {e}") from e
    ...
    def _create_git_worktree(self, worktree_path: Path, branch_name: str, base_branch: str) -> None:
        ...
        try:
            ...
            raise GitWorktreeError(f"Failed to create git worktree: {e}") from e
        except OSError as e:
            raise GitWorktreeError(f"Filesystem error creating worktree: {e}") from e
...
class GitIntegration:
    ...
    def remove_worktree(self, worktree_info: WorktreeInfo) -> None:
        """Remove a worktree.

        Args:
            worktree_info: Worktree information to remove
        """
        self.worktree_manager.cleanup_worktree(worktree_info)
