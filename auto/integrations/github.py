...
from auto.models import GitHubRepository, Issue, IssueProvider, IssueStatus, PullRequest
from auto.utils.logger import get_logger
from auto.utils.shell import ShellError, check_command_exists, run_command
...
class GitHubIntegration:
    ...
    def fetch_issue(self, issue_id: str, repository: GitHubRepository | None = None) -> Issue:
        ...
        try:
            ...
            if "could not resolve to an Issue" in e.stderr:
                raise GitHubIssueError(
                    f"Issue #{clean_issue_id} not found in {repository.full_name}"
                ) from e
            elif "HTTP 404" in e.stderr:
                raise GitHubIssueError(
                    f"Repository {repository.full_name} not found or not accessible"
                ) from e
            else:
                raise GitHubIssueError(
                    f"Failed to fetch issue #{clean_issue_id}: {e.stderr}"
                ) from e
        except json.JSONDecodeError as e:
            raise GitHubIssueError(f"Failed to parse issue data: {e}") from e
    ...
    def list_issues(
        ...
        try:
            ...
            result = run_command(" ".join(cmd_parts), check=True, timeout=30)
            ...
            if "HTTP 404" in e.stderr:
                ...
                raise GitHubIssueError(f"Failed to list issues: {e.stderr}") from e
        except json.JSONDecodeError as e:
            raise GitHubIssueError(f"Failed to parse issues data: {e}") from e
    ...
    async def create_pull_request(
        ...
        """
        from auto.models import PRStatus, PullRequest
        ...
        try:
            ...
            raise GitHubIntegrationError(f"Failed to create PR: {e.stderr}") from e
        except Exception as e:
            raise GitHubIntegrationError(f"Failed to create PR: {e}") from e
    ...
    async def add_pr_comment(self, repository: str, pr_number: int, comment: str) -> None:
        ...
        Raises:
            GitHubIntegrationError: If adding comment fails
        """
        try:
            logger.debug(f"Adding comment to PR #{pr_number} in {repository}")

            # Write comment to temporary file to avoid argument length limits
            import tempfile
            ...
            try:
                # Use gh pr comment command with body-file to handle long comments
                cmd_args = [
                    "gh",
                    "pr",
                    "comment",
                ...
                logger.debug(f"Successfully added comment to PR #{pr_number}")
            ...
            raise GitHubIntegrationError(f"Failed to add comment to PR: {e.stderr}") from e
        except Exception as e:
            raise GitHubIntegrationError(f"Failed to add comment to PR: {e}") from e
...
def detect_repository() -> GitHubRepository | None:
                ...
                try:
                    ...
                    if branch_result.success:
                        ...
                        master_result = run_command(
                            "git ls-remote --heads origin master", check=False
                        )
